...
/**
	...
	Main process (together with Function kmer_ext):
		...
		3. If next kmers is ambiguous, and there are junction reads supporting branches:
			3.1 extend the current template (repeat step 2)
...
Function template_ext(template, hash_table, query, direction):
	...
	while True:
		...
		if (second_c != -1):
			...
			max_query = shift_query(query, max_c, direction)
			max_branch = try_short_tpl_ext(max_query, direction, 5)
			branch_query = shift_query(query, second_c, direction)
			second_branch = try_short_tpl_ext(branch_query, direction, 5)

			// If there is probable branching
			if (max_branch != second_branch): 
				second_branch = try_short_tpl_ext(branch_query, direction, hash_table.read_length - 5)
				has_junction_read = find_junction_reads(template, second_branch)
				...
				if (has_junction_read):
					...
					template_ext(template, hash_table, max_query, direction)
					print "Extending the branch at %d" % branching_locus
					// For the branch, extend to only one direction, because another direction is extended by main template already
					second_branch = second_c
					template_ext(second_branch, hash_table, branch_query, direction)
				...
				// It is just an ambiguous base because of sequencing errors, etc.
...
Function try_short_tpl_ext(query, direction, max_len):
	...
	for (i = 0; i < max_len; i++):
		...
		if (c == -1):
			...
			extend_template(branch_template, c, direction)
...
/**
	...
	E.g.:	read_length: 15
			...
			junction_reads:               CCCCCCCCCCTTTTT
			                                 CCCCCCCTTTTTTTT 
**/
Function find_junction_reads(main_template, branch, hash_table):
	// Get the tail of the main template
	main_template_tail = get_tail(main_template, hash_table.read_length - 5)
	// Concat the two branches, temporarily
	probable_junction_template = concat(main_template, branch)
	junction_reads = []
