 ...
 */
void bwa_cal_pac_pos_core(const bwt_t *forward_bwt, const bwt_t *reverse_bwt,
		bwa_seq_t *seq, const int max_mm, const float fnr) {
	int max_diff;
	...
	if (seq->strand) { // reverse strand only
		...
		seq->pos = reverse_bwt->seq_len - (bwt_sa(reverse_bwt, seq->sa)
				+ seq->len);
		seq->seQ = seq->mapQ = bwa_approx_mapQ(seq, max_diff);
	}
}

void pe_cal_pac_pos(const bwt_t *bwt_0, const bwt_t *bwt_1, int n_seqs,
	...
	for (i = 0; i != n_seqs; ++i) {
		if (seqs[i].strand)
			bwa_cal_pac_pos_core(bwt_0, 0, &seqs[i], max_mm, fnr);
		for (j = 0; j < seqs[i].n_multi; ++j) {
			bwt_multi1_t *p = seqs[i].multi + j;
			if (p->strand)
				p->pos = bwt_sa(bwt_0, p->pos);
		}
	}
	//	bwt_destroy(bwt);
	...
	for (i = 0; i != n_seqs; ++i) {
		if (!seqs[i].strand)
			bwa_cal_pac_pos_core(0, bwt_1, &seqs[i], max_mm, fnr);
		for (j = 0; j < seqs[i].n_multi; ++j) {
			bwt_multi1_t *p = seqs[i].multi + j;
			if (!p->strand)
				p->pos = bwt_1->seq_len - (bwt_sa(bwt_1, p->pos) + seqs[i].len);
		}
	}
	//	bwt_destroy(bwt);
}

void bwa_cal_pac_pos(const char *prefix, int n_seqs, bwa_seq_t *seqs,
...
static bwa_cigar_t *refine_gapped_core(bwtint_t l_pac, const ubyte_t *pacseq,
		int len, const ubyte_t *seq, bwtint_t *_pos, int ext, int *n_cigar,
		int is_end_correct) {
	bwa_cigar_t *cigar = 0;
	ubyte_t *ref_seq;
...
char *bwa_cal_md1(int n_cigar, bwa_cigar_t *cigar, int len, bwtint_t pos,
		ubyte_t *seq, bwtint_t l_pac, ubyte_t *pacseq, kstring_t *str, int *_nm) {
	bwtint_t x, y;
	int z, u, c, nm = 0;
	str->l = 0; // reset
	x = pos;
	...
	if (s->strand == 0) { // forward
		...
		if (s->cigar && __cigar_op(s->cigar[0]) == FROM_S) { // the first is S
			...
			if (s->cigar == 0) {
				...
				++s->n_cigar;
				s->cigar = realloc(s->cigar, s->n_cigar * sizeof(bwa_cigar_t));
				memmove(s->cigar + 1, s->cigar, (s->n_cigar - 1)
						* sizeof(bwa_cigar_t));
			}
...
void bwa_refine_gapped(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs,
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		for (j = 0; j < s->n_multi; ++j) {
			...
			if (q->gap == 0)
				continue;
			q->cigar = refine_gapped_core(bns->l_pac, pacseq, s->len,
					q->strand ? s->rseq : s->seq, &q->pos, (q->strand ? 1 : -1)
							* q->gap, &n_cigar, 1);
			q->n_cigar = n_cigar;
		}
		if (s->type == BWA_TYPE_NO_MATCH || s->type == BWA_TYPE_MATESW
				|| s->n_gapo == 0)
			continue;
		s->cigar = refine_gapped_core(bns->l_pac, pacseq, s->len,
				s->strand ? s->rseq : s->seq, &s->pos, (s->strand ? 1 : -1)
						* (s->n_gapo + s->n_gape), &s->n_cigar, 1);
	}
		...
		for (i = 0; i < n_seqs; ++i) {
			...
			for (j = 0; j < s->n_multi; ++j) {
				...
				if (q->gap == 0)
					continue;
				free(q->cigar);
				q->cigar = refine_gapped_core(bns->l_pac, ntpac, s->len,
						q->strand ? s->rseq : s->seq, &q->pos, (q->strand ? 1
								: -1) * q->gap, &n_cigar, 0);
				q->n_cigar = n_cigar;
			}
			if (s->type != BWA_TYPE_NO_MATCH && s->cigar) { // update cigar again
				free(s->cigar);
				s->cigar
						= refine_gapped_core(bns->l_pac, ntpac, s->len,
								s->strand ? s->rseq : s->seq, &s->pos,
								(s->strand ? 1 : -1) * (s->n_gapo + s->n_gape),
								&s->n_cigar, 0);
			}
		}
	}
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		if (s->type != BWA_TYPE_NO_MATCH) {
			...
			s->md = bwa_cal_md1(s->n_cigar, s->cigar, s->len, s->pos,
					s->strand ? s->rseq : s->seq, bns->l_pac, ntbns ? ntpac
							: pacseq, str, &nm);
			s->nm = nm;
		}
	}
...
alignment *bwa_print_sam1(const bntseq_t *bns, bwa_seq_t *p,
		const bwa_seq_t *mate, int mode, int max_top2) {
	int j;
	alignment *align = (alignment*) malloc(sizeof(alignment));
	alg *algs = 0, *al;
	align->n_algs = 0;
	if (p->type != BWA_TYPE_NO_MATCH || (mate && mate->type
			!= BWA_TYPE_NO_MATCH)) {
		...
		if (p->type != BWA_TYPE_NO_MATCH && p->pos + j
				- bns->anns[seqid].offset > bns->anns[seqid].len)
		...
		algs = (alg*) calloc(p->n_multi + 1, sizeof(alg));
		algs->q_id = (char*) malloc(strlen(p->name) + 1);
		algs->r_id = (char*) malloc(strlen(bns->anns[seqid].name) + 1);
		strcpy(algs->q_id, p->name);
		algs->flag = flag;
		strcpy(algs->r_id, bns->anns[seqid].name);
		algs->pos = (int) (p->pos - bns->anns[seqid].offset + 1) - 1; // Make it 0-based.
		algs->len = p->len;
		align->n_algs = p->n_multi + 1;
		...
		if (p->type != BWA_TYPE_NO_MATCH) {
			...
			if (p->n_multi) {
				...
				for (i = 0; i < p->n_multi; ++i) {
					...
					al = &algs[i + 1];
					al->q_id = (char*) malloc(strlen(p->name) + 1);
					al->r_id
							= (char*) malloc(strlen(bns->anns[seqid].name) + 1);
					strcpy(al->q_id, p->name);
					strcpy(al->r_id, bns->anns[seqid].name);
					al->strand = q->strand ? '-' : '+';
					al->pos = (int) (q->pos - bns->anns[seqid].offset + 1) - 1; // Make it 0-based.
...
alignment *pe_sai2sam_se_core(const gap_opt_t *opt, bwa_alg *b_alg,
		const bwt_t *bwt_0, const bwt_t *bwt_1, const bntseq_t *bns,
		bwa_seq_t *seqs, int n_occ) {
	bwt_aln1_t *aln = 0;
	clock_t t;
	alignment *align;
	int n_seqs = 1;
	...
	// read alignment
	aln = (bwt_aln1_t*) realloc(aln, sizeof(bwt_aln1_t) * b_alg->n_algs);
	bwa_aln2seq_core(b_alg->n_algs, b_alg->algs, seqs, 1, n_occ);
	pe_cal_pac_pos(bwt_0, bwt_1, n_seqs, seqs, opt->max_diff, opt->fnr); // forward bwt will be destroyed here
	bwa_refine_gapped(bns, n_seqs, seqs, 0, 0);
	align = bwa_print_sam1(bns, seqs, 0, opt->mode, opt->max_top2);
	bwa_free_read_seq(n_seqs, seqs);

	return align;
}

alg *bwa_sai2sam_se_core(const char *prefix, const char *fn_sa,
		const char *fn_fa, int n_occ) {
	...
	while ((seqs = bwa_read_seq(ks, 0x40000, &n_seqs, opt.mode, opt.trim_qual))
		...
		for (i = 0; i < n_seqs; ++i) {
			algs = bwa_print_sam1(bns, seqs + i, 0, opt.mode, opt.max_top2);
		}
		//		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
