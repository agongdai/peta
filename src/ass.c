...
char *get_out_file(const char *file_name) {
	char *name = (char*) calloc(512, sizeof(char));
	strcat(name, opt->out_root);
	strcat(name, "/");
...
void fill_in_gap(edge *left_eg, edge *right_eg, const int reason_gap,
		const hash_table *ht, const int ori, const int used_eg_id) {
	...
	if (ol || (reason_gap > (0 - opt->rl) && reason_gap < MIN_OL)) {
	...
	if (used_eg_id != -1) {
		show_debug_msg(__func__,
				"Reaching some read used in the main edge, but not overlapped! \n");
		return;
	}
	...
	while (1) {
		...
		if (c[1] != INVALID_CHAR) {
			if (bases_sup_branches(cur_pool, ori, BASES_SUPPORT_THRE)) { // Check one base ahead the cursor
				...
				show_debug_msg(__func__,
						"Denied branching: a:c:g:t:n => %d:%d:%d:%d:%d \t %d:%d:%d:%d:%d [%d, %d]\n",
...
int linear_ext(edge *ass_eg, const hash_table *ht, bwa_seq_t *cur_query,
	...
	eg_gap *exi_gap = NULL;
	readarray *tried_mates = NULL;

	tried_mates = g_ptr_array_sized_new(max_try_times);
	ori_len = ass_eg->len;
	...
	while (max_try_times-- > 0 && len_le == len_init) {
		...
		if (!mate) {
			readarray_unfrozen(tried_mates); // Free inside
			show_debug_msg(__func__, "Mates are empty, return \n");
			return 0;
		}
		// If the mate has been tried before, ignore it
		if (readarray_find(tried_mates, mate) != NOT_FOUND)
			continue;
		readarray_frozen(tried_mates, mate);

		if (is_repetitive_q(mate)) {
			show_debug_msg(__func__, "Mates are repetitive, ignore \n");
		...
		if (len_le > len_init) {
			...
			if (!ori && m2 && m2->type == REP_EXTEND) {
				used_eg_id = m2->used->contig_id;
			}
		}
	...
	if ((reason_gap < (0 - opt->mean) || reason_gap < (0 - opt->rl - 2))) {
		show_debug_msg(__func__,
	...
	upd_reads_by_ol(ht->seqs, ass_eg, opt->nm);
	destroy_eg(m_eg);
	return extended;
}
...
int post_vld_branch(edge *ass_eg, edge *child, const hash_table *ht,
	...
	if (main_acc_len < opt->mean || child_acc_len < opt->mean) {
		valid = 0;
	} else {
		if (ori)
	...
	while (1) {
		...
		if (msg->type == REP_QUE || msg->type == NOT_EXTEND
			...
			if (used->contig_id >= 0 && used->contig_id != ass_eg->id && !ori) {
				...
				show_debug_msg(__func__,
						"ass_eg->r_shift: %d, used->shift: %d, used->cursor: %d \n",
						ass_eg->r_shift, used->shift, used->cursor);
				show_debug_msg(__func__,
...
edge *pe_ass_ctg(roadmap *rm, bwa_seq_t *read, hash_table *ht) {
	...
	upd_reads_by_ol(ht->seqs, cur_eg, opt->nm);
	for (i = 0; i < all_edges->len; i++) {
...
int pe_ass(int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv, "n:k:r:a:m:s:p:d:b:o:")) >= 0) {
		switch (c) {
		case 'n':
			opt->nm = atoi(optarg);
			break;
	...
	fprintf(stderr, "out_root = %s \n", opt->out_root);

	if (mkdir(opt->out_root)) {
		show_msg(__func__, "Output folder %s is created.\n", opt->out_root);
	} else {
