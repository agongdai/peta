...
extern "C" {
	...
	void find_match_mates(hash_table *ht, pool *p, GPtrArray *near_tpls, tpl *t, int tail_len,
			int mismatches, int ori);
	void find_hashed_mates(hash_table *ht, pool *p, GPtrArray *near_tpls, tpl *t, int full_tail_len,
			int mismatches, int ori);
