...
void build_kmers(const char *fa_fn, const char *out_fn, const int k) {
	...
	uint32_t n_reads = 0, i = 0, j = 0, n_kmers = 0;
	uint64_t *mer_v = 0;
	mer *m = NULL;
	mer *exist_m = NULL;
	GHashTable* hash = g_hash_table_new_full(g_int64_hash, g_int64_equal, (GDestroyNotify) free_int64, NULL);
	...
	g_ptr_array_sort(kmer_list, (GCompareFunc) cmp_kmer_by_seq);
	for (i = 0; i < 100; i++) {
		m = g_ptr_array_index(kmer_list, i);
		show_debug_msg(__func__, "Kmer: %" ID64 ", %d \n", m->s, m->count);
	}
	fwrite(&n_kmers, sizeof(uint32_t), 1, out);
	fwrite(kmer_list->pdata, sizeof(mer), kmer_list->len, out);
